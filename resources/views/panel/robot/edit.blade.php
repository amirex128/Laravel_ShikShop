							...
							<div class="form-wrap">
								...
								<div class="panel-body">
									<div class="row">
										...
										<div class="col-12">
											<form  id="form-save" class="form-row" method="post" action="/panel/robot/{{$robot->id}}">
												@csrf
												@method('put')
												<div id="form-section">
													<div class="form-group col-sm-6">
														<label for="category-destination">دسته بندی مبدا</label>
														<input value="{{old('origin',$robot->origin)}}" name="origin" id="category-destination" type="text"
														       class="form-control">
													</div>
													...
													<div class="form-group col-sm-6">
														...
														<select name="destination" id="category-destination"
															...
															@foreach($categories as $category)
																<option {{$robot->category->id == $category->id ? "selected":""}} value="{{$category->id}}">{{$category->title}}</option>
															@endforeach
														</select>
													</div>
												</div>
												...
												<div class="col-12">
													<div class="row">
														<div class="col-sm-6 text-center">
															<a href="panel/robot/create" id="add" class="btn btn-success"> افزودن
															</a>
															<div onclick="document.getElementById('form-delete').submit()" id="delete" class="btn btn-danger"> حذف
															</div>
														
														</div>
														<div class="col-sm-6 text-center">
															<button id="save" class="btn btn-primary"> ذخیره
															</button>
														</div>
													</div>
												</div>
											...
											<form id="form-delete" method="post" action="panel/robot/{{$robot->id}}">
												@method('delete')
											</form>
